...
```json
{
  "daily": [
    {"date": "2026-06-01", "sessions": 78, "total_users": 62, "new_users": 48,
     ...
     "avg_engagement_time": 57.3, "bounce_rate": 0.45, "page_views": 145, "conversions": 0,
     "sessions_excl_auto": 71, "engaged_sessions_excl_auto": 43}
  ],
  "channels": [
...
## Automated traffic (`sessions_excl_auto` / `engaged_sessions_excl_auto`)

`avg_engagement_time` is GA4's `averageEngagementTimePerSession` — the SDK's foreground
visibility timer. Prefer it over `avg_session_duration`, which is just
(last event − first event) and collapses to ~0 for single-event sessions.

`sessions_excl_auto` and `engaged_sessions_excl_auto` are the same counts with sessions
from known cloud/datacenter egress locations removed. Those sessions register
essentially zero engagement time and are almost certainly headless crawlers — they
survive GA4's built-in bot filter because they execute JavaScript.

The exclusion list, applied identically to every period so comparisons stay valid:

    Singapore, Ashburn, Council Bluffs, Des Moines, Boardman, The Dalles,
    Urumqi, Tehran, Amsterdam, Prague, Fairfax

This is a geography heuristic, not proof. It was derived from GA4 city × engagement-time
data on 2026-07-25, where these locations averaged 0.0–1.7s per session against a site
average near 55s. Singapore alone went from 30 sessions in the 28 days before the
relaunch to 213 after — 12.6% of all site traffic at 0.061s per session. Confirming it
properly needs server or CDN logs (user-agent and IP); until then both the reported and
the excluded figures are shown side by side rather than silently substituted.

Both fields are optional and nullable. Snapshots written before this existed simply
carry no value, and the importer preserves any figure already in the database rather
